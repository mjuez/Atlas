...
class ButtonsContainer extends ToggleElement {
    constructor(element) {
        super(element)
        if (!(typeof element.id === 'string')) element.id = "ButtonsContainer";
        this.id = element.id;
        this.buttons = {};
        this.buttonGroups = {};
        this.nButtons = 0;
    ...
    addButton(options) {
        ...
        if (options.toggle) {
            btn.onclick = (arg) => {
                let b = document.getElementById(`${this.id}Button${options.id}`);
                let done = false;
                if (b.className.includes('active')) {
                    b.className = `btn ${options.className}`;
                    if (typeof options.action.deactive === 'function') {
                        options.action.deactive(b);
                        done = true;
                    }
                } else {
                    b.className = `btn ${options.className} active`;
                    if (typeof options.action.active === 'function') {
                        options.action.active(b);
                        done = true;
                    }
                }
    ...
    removeButton(id, force) {
        if (typeof id === 'string') {
            if (this.buttons[id]) {
                let btn = document.getElementById(`${this.id}Button${id}`);
                if (btn) {
    ...
    addButtonGroup(options, buttons) {
        ...
        grp.className = `btn-group ${options.className}`;
