...
class ListGroup extends ToggleElement {
    ...
    addItem(options) {
        ...
        if (options.body) {
            if (typeof options.body.appendChild === 'function') {
                body.appendChild(options.body);
                item.body = new ToggleElement(options.body);
            } else if (options.body instanceof ToggleElement) {
    ...
    addSearch(options) {
        ...
        input.onsearch = () => {
            ...
            let val = input.value;
            for (let it in this.items) {
                if (this.items[it].element.getElementsByTagName('STRONG')[0].innerHTML.toLowerCase().includes(val.toLowerCase()) || val === '') {
                    this.items[it].show();
                } else {
                    this.items[it].hide();
                }
