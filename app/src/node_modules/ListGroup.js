...
class ListGroup extends ToggleElement {
    ...
    addItem(options) {
        ...
        if(options.actions){
            rightContainer.appendChild(options.actions);
        }

        if(options.contextMenu){
            li.oncontextmenu = (e) => {
                options.contextMenu.popup();
            };
        }
        ...
        if (options.body) {
            if (options.body instanceof ToggleElement) {
                detailsContainer.appendChild(options.body.element);
            } else if (typeof options.body === 'string') {
                let cont = document.createElement('P');
                cont.innerHTML = options.body;
                detailsContainer.appendChild(cont);
            } else if (typeof options.body.appendChild === 'function') {
                detailsContainer.appendChild(options.body);
            }
                ...
                if (li.classList.contains('active')) {
                    ...
                    if (options.toggle.justOne) {
                        ...
                        if (options.toggle.expand) {
                            if (item.details) {
                                this.hideAllDetails();
                                let caret = '<i class="fa fa-caret-down" aria-hidden="true"></i>';
                                leftContainer.innerHTML = caret;
                                item.details.show();
                            }
                        }
