...
class ObjectDetectionTask extends Task {
    ...
    constructor(details, isFolder, gui) {
    ...
    run(runPath) {
        ...
        this.showModal((modal, params) => {
            let args = `${this.isFolder}#${runPath}#${params.rmin}#${params.rmax}#${params.by}#${params.thrMethod}#${params.min}#${params.max}#${params.fraction}#${params.toll}#${params.path}`;
            ...
            this.childProcess.stdout.on('data', (data) => {
              if (data.startsWith('_DONE_')) {
                  tot++
                  this.progress.innerHTML = `${tot} image processed`;
              }
            });
            ...
            this.childProcess.on('close', (code) => {
                let promise = new Promise((resolve) => {
                    let notification;
                    if (code == 0) {
                        Util.Layers.createJSONConfiguration(runPath, params.path, this.isFolder, layerType, (config) => {
            ...
            this.gui.notify(`Object detection task started.`);
            modal.destroy();
        });
    }
    ...
    DOMActions() {
        ...
        buttonsContainer.addButton({
            id: "LoadMap00",
            text: "Add layer to a map in workspace",
            action: () => {
                Task.Utils.showMapSelector(gui, this.jsonFile);
            },
    ...
    _createDOMElement() {
        ...
        this.progress.innerHTML = `0 image processed`;
        this.customActions.appendChild(this.progress);
    }
