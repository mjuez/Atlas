...
Util.Layers = class {
    ...
    static createJSONConfiguration(sourcePath, destinationPath, isFolder, layerType, next) {
        let template;
            ...
            if (isFolder) {
                fs.readdir(sourcePath, (err, files) => {
                    ...
                    Promise.all(allStatsPromises).then(() => {
                        ...
                        let yMin = Math.min(...xValues);
                        let yMax = Math.max(...xValues);
            ...
            switch (layerType) {
                case `points`:
                    jsonConfig = {
                        ...
                        tileSize: size[0],
                        size: size[1],
                        pointsUrlTemplate: `${destinationPath}${path.sep}points${path.sep}points_${filename.replace(" ","_")}.csv`
                    }
                    break;
                ...
                case `pixels`:
                    jsonConfig = {
                        ...
                        tileSize: size[0],
                        size: size[1],
                        pixelsUrlTemplate: `${destinationPath}${path.sep}holes_pixels${path.sep}holes_${filename.replace(" ","_")}.txt`
                    }
                    break;
            }
